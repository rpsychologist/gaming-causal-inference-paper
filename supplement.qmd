...
code-fold: true
code-tools: 
    source: https://github.com/rpsychologist/gaming-causal-consistency-paper/index.qmd
...
- [Preprint](/)
- [GitHub respository](https://github.com/rpsychologist/gaming-causal-consistency-paper/)
- [OSF repository](https://osf.io/qfw23/)

::: {.content-visible when-format="pdf"}
A HTML version of this document can also be viewed at [https://rpsychologist.github.io/gaming-causal-consistency-paper](https://rpsychologist.github.io/gaming-causal-consistency-paper)
:::
...
When then simulate a large data set with with a high proportion (85%)
...
that the observed difference is outcomes is caused by the confounder.
...
Unsuprisingly, we get the same biased effect if we run a regression without including
the confounder `U`.
...
function to generate simulated data with 4 version, 2 version among the
unexposed and 2 versions for the exposed, both exposure and the version
is infleunced by binary confounder (W).
...
exposed 80% are men, and among the exposed men 90% get K = 2, and among
the unexposed men 90% also get K = 2.
...
#| tbl-cap: Summary of the unexposed group
# control
d %>%
    filter(A == 0) %>%
    summarise(
...
We also see that the overall values are correct, inluding the biased
effect.

```{r, tbl-summary-overal}
#| tbl-cap: Summary overal
pr_A <- mean(d$A)
pooled_SD <- sqrt(
...
#| echo: false
p <- ggplot(
    mutate(d, A = factor(A, labels = c("Control", "Gamers"))),
    aes(A, y, fill = "Overall", color = "Overall", group = A)
) +
    geom_violinhalf(
        # color = NA,
        alpha = 0.40,
    ...
    lims(y = c(-2.5, 2.5)) +
    labs(y = "Mental health", x = "Observed outcomes") +
    scale_color_manual(values = c("Control" = col1, "Gamers" = col2, "Overall" = "black")) +
    scale_fill_manual(values = c("Control" = col1, "Gamers" = col2, "Overall" = "black")) +
    guides(
...
ggsave("figures/figure-version-confounding-0.svg", p, width = 5, height = 4)
p
```
...
p <- ggplot(
    ...
    scale_color_manual(values = c("1" = col1, "2" = col2, "Overall" = "black")) +
    scale_fill_manual(values = c("1" = col1, "2" = col2, "Overall" = "black")) +
    guides(
        colour = guide_legend("Variant"),
        fill = guide_legend("Variant")
    ) +
...
If look at the data within each strata of the counfounder, in @fig-exposure-version-stratified we see that
overall impact among the older group is positive, whereas it is negative
among men. This is interely driven by yound and old people being exposed
...
ggsave("figures/figure-version-confounding-2.svg", p2, width = 5, height = 4)
ggsave("figures/figure-version-confounding-2.png", p2, width = 5, height = 4, bg = "white")
p2
```
...
gamers vs if noone was playing games. 
...
In @fig-exposure-version-ATE we plot the counterfactuals, and we see that the ATE is 0. However, we'd get a much more nuanced answer if we had data on the exposure version.
...
ggsave("figures/figure-version-confounding-3.svg", p, width = 5, height = 4)
p
```

### Average treatment effect on the treated (ATT).

It's possible that most people do not have the ATE in mind wh en
thinking about the causal effect of video games. It might be more
intuitive to think what would happend to the population of gamers if
...
We can calculate the ATT using g-computation, the estimated ATT is shown in @tbl-ATT.
...
```{r, tbl-ATT}
#| tbl-cap: ATT estimate caclulate using g-computation
d_pred %>%
    summarize(
        ATT = mean(y_a1 - y_a0)
    ) %>%
...
we can plot it anyway. The dashed y0 curves show the obseved data. We see
that if we compare the gamers with the non-gamers we get a
counterfactual comparison that is wrong, the versions if Y0 is weigted
